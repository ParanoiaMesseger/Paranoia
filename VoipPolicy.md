...
**Важно про forward-compat:** строгая проверка RESERVED-битов означает, что любое будущее расширение wire-format (новый флаг, новый stream_id) требует одновременного апдейта обоих концов. Mixed-version звонки распадутся. Это сознательный компромисс ради валидации схемы.
...
- `MAX_DATAGRAM = 1200` (MTU-safe для cellular: LTE/5G c GTP/PPPoE/VPN-туннелями операторов часто имеют path-MTU 1280–1400; 1200 байт UDP payload + 28 байт IP/UDP-заголовков = 1228 байт «по проводу», влезает в IPv6 minimum MTU 1280 без фрагментации).
- `MAX_FRAGMENT_PAYLOAD = 1200 − 16 (header) − 16 (Poly1305 tag) = 1168` байт.
...
| Inband FEC | включён |
| DTX | **выключен** (молчание кодируется минимальным фреймом — ~7 байт). Так Qt-сторона держит непрерывный 20-мс поток и не зависит от Rust-keepalive'а для NAT-mapping'а. |
...
При bind на `0.0.0.0:port` сокет создаётся как IPv6 dual-stack (`[::]:port` с `IPV6_V6ONLY=false`). Это нужно для мобильных операторов с IPv6-only сетью и DNS64 — `getaddrinfo("paranoia.example.com")` возвращает синтезированный `[64:ff9b::5d64:cf97]`, IPv4-only сокет упал бы с `EAFNOSUPPORT` на `send_to`. Dual-stack сокет принимает и шлёт оба family одновременно.

Из-за этого dual-stack-сокета STUN/TURN client не сверяет `from.ip()` с `server.ip()` — при NAT64 запрос идёт на v4-mapped IPv6, а ответ может прийти с другого IP-mapping'а. От подмены защищает только `transaction_id` (12 случайных байт).
...
STUN-сервер задаётся через переменную окружения `PARANOIA_STUN_SERVER` (значения `disable|off|none` явно отключают STUN), либо через compile-time `-DPARANOIA_DEFAULT_STUN_SERVER`. При активной session UI-клиент автоматически использует `host(server_url):3478` как STUN и TURN. Если ничего не задано и нет активной сессии — фолбэк `stun.l.google.com:19302` для STUN; TURN-фолбэка нет.

TURN-сервер задаётся аналогично через `PARANOIA_TURN_SERVER`; если пусто и нет активной сессии — TURN-fallback отключён (звонок работает только при successful direct/reflexive).
