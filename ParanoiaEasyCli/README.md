...
- **консольный клиент** Paranoia — отправка/приём сообщений и файлов из терминала,
  плюс интерактивный TUI-мессенджер (подкоманда `tui`);
...
| Команда | Что делает |
|---------|-----------|
| `send --username <sid> --peer <имя\|sid> --text <…> [--topic <имя>]` | отправить текстовое сообщение (опц. в тему/ветку диалога) |
| `react --username <sid> --peer <…> --message-id <id> --emoji <эмодзи>` | поставить эмодзи-реакцию на сообщение (например прогресс-статус 👀/🤔/✍️/✔️) |
| `receive --username <sid> --peer <…> [--long-poll-ms N] [--topic <имя>]` | забрать новые сообщения (двигает курсор; опц. фильтр по теме) |
| `watch --username <sid> --peer <…> [--interval N] [--long-poll-ms N] [--topic <имя>]` | непрерывный приём (long-poll или поллинг; опц. фильтр по теме) |
| `send-file --username <sid> --peer <…> --path <файл> [--topic <имя>]` | отправить файл/картинку (канал авто по размеру) |
...
| `tui [--username <sid>]` | интерактивный TUI-мессенджер: диалоги, лента, ввод, live-приём, реакции, вложения |
| `profile-name --username <sid> --name <ник>` | задать локальный ник профиля (виден в выборе профиля в TUI) |

`--peer` принимает отображаемое имя (резолвится в server_id через `names`) или сам
server_id. `--username` — **всегда server_id** (см. ниже).

### Темы (ветки) диалога
Один диалог можно вести несколькими параллельными ветками («темами»). Тема
создаётся неявно при первой отправке (`send … --topic <имя>`) и появляется у
собеседника автоматически; пустая тема — «Главная».

| Команда | Что делает |
|---------|-----------|
| `topic list --username <sid> --peer <…>` | список тем диалога: имя + число сообщений |
| `topic delete --username <sid> --peer <…> --topic <имя>` | удалить тему целиком (у обеих сторон) |
| `topic trim --username <sid> --peer <…> [--keep N] [--topic <имя>] [--dry-run]` | оставить последние N сообщений (по умолч. 20) в теме или во всех темах |
...
- **`topics`** — список тем (веток) диалога: имя + число сообщений;
- **`set_channel_topic`** — привязать сессию к одной теме (фильтр приёма в channel-режиме + дефолт темы для `send`); пока тема не задана — приём остановлен (агенту приходит напоминание задать тему), сессия НЕ слышит чужие ветки; тема `Главная` — главная безтемовая ветка; без аргумента — сброс в «не задано»;
