...
public:
    ...
    // filePath: путь к файлу экспорта; расшифровывается device private_key
...
private:
    ...
    static QList<Dialog> loadDialogsFromPath(const QString &path);
    static void saveDialogsToPath(const QString &path, const QList<Dialog> &dialogs);
