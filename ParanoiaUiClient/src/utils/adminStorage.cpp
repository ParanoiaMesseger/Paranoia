...
QFuture<bool> admin::Admin::regUser(const QString &username, const QString &pubkey) const
{
    ...
    const QString reserveUrlsJson = Utils::reserveServerUrlsJson(Utils::normalizedServerUrls(reserveServerUrls, domain));
...
void admin::Admin::initAdmins()
{
    ...
    if (doc.isArray()) {
        for (const auto &value : doc.array()) {
            ...
            const QString domain        = Utils::normalizedServerUrl(obj.value("url").toString(obj.value("domain").toString()));
...
void admin::Admin::saveAdmins()
{
    ...
    for (const auto &admin : admins) {
        ...
        obj["reserve_server_urls"]   = Utils::stringListToJsonArray(
            Utils::normalizedServerUrls(admin.reserveServerUrls, admin.domain));
        arr.append(obj);
    }
