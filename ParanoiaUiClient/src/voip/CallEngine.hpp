...
namespace paranoia::voip
{
    ...
    public slots:
        /// Вызывается через `QMetaObject::invokeMethod(this, "enqueueIncomingFrame",
        /// Qt::QueuedConnection, ...)` из произвольного потока FFI: помещает
        /// фрейм в jitter buffer. Реальное декодирование/воспроизведение
        /// происходит на тике [`playbackTimer_`].
        void enqueueIncomingFrame(const QByteArray &opus, quint64 sequence);

        /// Вызывается из FFI на каждый расшифрованный видеофрагмент.
        /// Реассемблируется → декодируется → отдаётся в `remoteVideoSink`.
        /// Сейчас выполняется на main thread (QueuedConnection).
    ...
    private:
        ...
        bool sent_audio_packet_              = false;
        bool decoded_audio_packet_           = false;
        bool remote_video_active_            = false;
        bool media_received_                 = false;
        quint64 mic_frame_count_             = 0;
        quint64 sent_voice_packet_count_     = 0;
        quint64 received_voice_packet_count_ = 0;
        quint64 decoded_voice_frame_count_   = 0;
        ...
        // Реассемблер: накапливаем фрагменты NAL по rtp_timestamp.
        QByteArray reassembly_buffer_;
        quint32 reassembly_ts_       = 0;
        bool reassembly_active_      = false;
        quint64 reassembly_last_seq_ = 0;
