...
// Переиспользуемый редактор резервных доменов + резервных TURN-серверов. БЕЗ своего
// хедера и БЕЗ Flickable — встраивается в любой скролл-контейнер (страница резерва
// AddReserveDomainPage И окно «Настройки профиля», чтобы не плодить отдельные окна).
// targetType: "client" | "admin"; для "admin" секция TURN скрыта.
ColumnLayout {
    id: root
    spacing: 16
    ...
    Connections {
        ...
        function onReserveDomainCheckFinished(type, id, reserve, ok, msg, pingMs) {
            if (!root.matchesTarget(type, id)) return
            root.setStatus(reserve, { state: ok ? "ok" : "error", pingMs: pingMs, message: msg })
        }
